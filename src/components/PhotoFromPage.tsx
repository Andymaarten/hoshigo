...
export default function PhotoFromPage({
  ...
  label: string;
  /** every photo from the last page failed to load (and was removed from the picker) */
  allFailed?: boolean;
}) {
  ...
  const failedAfterLoad = state === "found" && allFailed;
    ...
    <div className="photo-from-page">
      <div className="search-row">
        ...
        {value && (
          <button
            type="button"
            className="text-btn input-clear"
            onClick={() => {
              setValue("");
              setState("idle");
      ...
      <span className={failedAfterLoad || state === "none" || state === "blocked" || state === "error" || state === "not_a_link" ? "error" : "hint"} aria-live="polite">
        {failedAfterLoad ? "That photo didn't load, so we took it out. Try another page." : message}
      </span>
    </div>
  );
}
