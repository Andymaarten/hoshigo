...
export function imageSrc(url: string | null | undefined, size: "normal" | "small" = "normal"): string | null {
  if (!url) return null;
  try {
    const u = new URL(url);
    ...
    if (u.hostname === "image.tmdb.org") return s.replace(/\/t\/p\/(original|w\d+)\//, size === "small" ? "/t/p/w154/" : "/t/p/w342/");
    return s;
  } catch {
    return null;
  }
